...
int main() {
    ...
    vector<vector<long long>> dp(na.size(),
                                 vector<long long>(nb.size(), numeric_limits<long long>::max()));
    dp[0][0] = 2;
    for (int i = 0; i < na.size(); i++) {
        for (int j = 0; j < nb.size(); j++) {
            if (i > 0) {
                dp[i][j] = dp[i - 1][j] + sum_a[na[i]] - sum_a[na[i - 1]] +
                           gcd(nb[j], b) * (na[i] - na[i - 1]);
            }
            if (j > 0) {
                dp[i][j] = min(dp[i][j],
                               dp[i][j - 1] + sum_b[nb[j]] - sum_b[nb[j - 1]] +
                                   gcd(na[i], a) * (nb[j] - nb[j - 1]));
            }
        }
    }

...
ostream & operator<<(ostream &os, const BigNumber& bn)
{
	for (auto it = bn.num.rbegin(); it != bn.num.rend(); it++) {
		if (it == bn.num.rbegin()) { os << *it; }
		else { os << setw(bn.WIDTH) << setfill('0') << *it; }
	}
	return os;
}
...
int main()
{
    ...
    sort(temp, temp + n, [] (const auto &a, const auto &b) { return a.first * a.second < b.first * b.second; });
    for (int i = 0; i < n; i++) { minister.push_back({BigNumber(temp[i].first), BigNumber(temp[i].second)}); }
    BigNumber ans{0};

...
pair<int, int> query(int a, int b)
{
    ...
    for (int j = 19; j >= 0; j--) {
        ...
        if (maxValue[b][j] > res1 && res1 > res2) { res2 = res1; }
        else if (res1 > maxValue[b][j] && maxValue[b][j] > res2) { res2 = maxValue[b][j]; }
    ...
    for (int j = 19; j >= 0; j--) {
        ...
        if (temp > res1 && res1 > res2) { res2 = res1; }
        else if (res1 > temp && temp > res2) { res2 = temp; }
        res1 = max({res1, temp});
        a = parent[a][j];
        b = parent[b][j];
    }
...
int main()
{
    ...
    for (int i = 0; i < m; i++) {
        ...
        else if (res.first != edge[i].w) { ans = min(ans, mst - res.first + edge[i].w); }
        else if (res.second != -1) { ans = min(ans, mst - res.second + edge[i].w); }
    }
    cout << ans << endl;
	return 0;
}

...
struct Card {
    ...
    explicit Card(int roleID, int targetID, CARD_TYPE type)
        : roleID(roleID), targetID(targetID), type(type) {}
...
struct Role {
    ...
    int hp;
    bool hasWeapon;
    enum ROLE_TYPE { MAIN, LOYAL, REBEL } type;
    enum ATTITUDE { BAD, BAD_LIKE, GOOD, NONE };
    list<Card> handCard;
...
enum GAME_RESULT { UNFINISHED, MAIN_WIN, REBEL_WIN };
...
int main() {
    ...
    for (int i = 0; i < pigNum; i++) {
        ...
        if (pig == "MP") {
            ...
            role.type = Role::LOYAL;
        } else {
            role.type = Role::REBEL;
        }
        for (int j = 0; j < 4; j++) {
            cin >> tempCard;
    ...
    for (auto &&role : allRole) {
        if (role.type == Role::MAIN) {
            role.attitude[0] = Role::GOOD;
        } else if (role.type == Role::LOYAL) {
            role.attitude[0] = Role::GOOD;
        } else {
            role.attitude[0] = Role::BAD;
        }
    }
...
Card Role::tryPlayInvalidation(const Card &lastCard) {
    ...
    for (auto iter = handCard.begin(); iter != handCard.end(); iter++) {
        if (!iter->valid || iter->type != Card::INVALIDATION ||
            getAttitude(lastCard.targetID) == NONE || getAttitude(lastCard.targetID) == BAD_LIKE) {
            continue;
        }
        if (lastCard.type == Card::INVALIDATION && getAttitude(lastCard.targetID) == BAD) {
            iter->targetID = id;
        } else if (lastCard.type != Card::INVALIDATION && getAttitude(lastCard.targetID) == GOOD) {
            iter->targetID = lastCard.targetID;
        } else {
            continue;
        }
        iter->valid = false;
        // cout << id << " played invalidation to " << iter->targetID << " for " <<
        // lastCard.targetID << "\n"; cout << *this << endl;
        return *iter;
    }
    return Card{};
...
bool Role::playArrowOrInvasion(Card &card) {
    ...
    for (int i = id + 1; i < allRole.size(); i++) {
        ...
        if (allRole[card.targetID].hp == 0 && !allRole[card.targetID].tryPlayPeachWhenDead() &&
            checkGameState() != UNFINISHED) {
            return false;
        }
        ...
        if (card.targetID == 0 && allRole[0].getAttitude(id) == NONE) {
            allRole[0].attitude[id] = BAD_LIKE;
        }
    }
    for (int i = 0; i < id; i++) {
        if (allRole[i].hp <= 0) { continue; }

...
bool dotInPolygon(const pair<T, T> &dot, const vector<pair<T, T>> &polygon) {
    ...
    for (int i = 0, j = n - 1; i < n; j = i++) {
        ...
        if (cmp(crossProduct(dot2Vec(dot, p1), dot2Vec(dot, p2)), 0) == 0 &&
            cmp(dotProduct(dot2Vec(p1, dot), dot2Vec(dot, p2)), 0) >= 0) {
            return true;
        }
        if (((cmp(p1.second, dot.second) > 0) != (cmp(p2.second, dot.second) > 0)) &&
            cmp(dot.first - p1.first,
                1.0 * (dot.second - p1.second) * (p1.first - p2.first) / (p1.second - p2.second)) <
                0) {
            flag = !flag;
        }
    }
...
double getArea(const pair<double, double> &p1,
        ...
        for (int i = n - 3; i >= 0; i--) {
            for (int j = i + 2; j < n; j++) {
                ...
                for (int k = i + 1; k < j; k++) {
                    if (ok(i, j, k, p)) {
                        dp[i][j] =
                            min(dp[i][j], max(dp[i][k], max(dp[k][j], getArea(p[i], p[j], p[k]))));
                    }
                }
            }
        }

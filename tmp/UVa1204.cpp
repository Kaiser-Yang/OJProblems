            ...
            for (int iReversed = 0; iReversed < 2; iReversed++) {
                ...
                for (int j = 0; j < s.size(); j++) {
                    ...
                    for (int jReversed = 0; jReversed < 2; jReversed++) {
                        cover[i][iReversed][j][jReversed] = getCover(s[i], iReversed, s[j], jReversed);
                    }
                }
            }
        }
            ...
            for (int j = 0; j < s.size(); j++) {
                ...
                for (int reversed = 0; reversed < 2; reversed++) {
                    ...
                    for (int k = 0; k < s.size(); k++) {
                        if (i & (1 << k)) { continue; }
                        for (int l = 0; l < 2; l++) {
                            dp[i | (1 << k)][k][l] = max(dp[i | (1 << k)][k][l], dp[i][j][reversed] + cover[j][reversed][k][l]);
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < s.size(); i++) {
            for (int reversed = 0; reversed < 2; reversed++) {
                if (dp[(1 << s.size()) - 1][i][reversed] != -1) {
                    ans = min(ans, sumLen - dp[(1 << s.size()) - 1][i][reversed] - cover[i][reversed][0][0]);
                }
            }
        }
        if (ans < 2) { ans = 2; }
